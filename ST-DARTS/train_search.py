...
import data_tryonserver_multisub_ROI
...
parser.add_argument('--nhidlast', type=int, default=32,
                    help='number of hidden units for the last rnn layer')
parser.add_argument('--lr', type=float, default=20,
                    help='initial learning rate')
parser.add_argument('--clip', type=float, default=0.25,#0.25
                    help='gradient clipping')
parser.add_argument('--epochs', type=int, default=20,
                    help='upper epoch limit')#50
parser.add_argument('--batch_size', type=int, default=128, metavar='N',
                    help='batch size')#256
parser.add_argument('--bptt', type=int, default=1,
                    help='sequence length')
parser.add_argument('--dropout', type=float, default=0.8,#0.75
                    help='dropout applied to layers (0 = no dropout)')
parser.add_argument('--dropouth', type=float, default=0,#0.25
                    help='dropout for hidden nodes in rnn layers (0 = no dropout)')
parser.add_argument('--dropoutx', type=float, default=0.1,#0.75
                    help='dropout for input nodes in rnn layers (0 = no dropout)')
parser.add_argument('--dropouti', type=float, default=0.1,#0.2
...
parser.add_argument('--max_seq_len_delta', type=int, default=1,
                    help='max sequence length')
parser.add_argument('--single_gpu', default=True, action='store_false', 
                    help='use single GPU')
parser.add_argument('--gpu', type=int, default=0, help='GPU device to use')
parser.add_argument('--unrolled', action='store_true', default=False, help='use one-step unrolled validation loss')  #unrolled is for the second-order derivative, or else is for the first-order derivative
parser.add_argument('--arch_wdecay', type=float, default=1e-3,#
                    help='weight decay for the architecture encoding alpha')
parser.add_argument('--arch_lr', type=float, default=3e-3,#0.001
...
if not args.continue_train:
    args.save = 'Gambling 0.8 0.1 0.1 search_1'#'search-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
corpus = data_tryonserver_multisub_ROI.Corpus()

eval_batch_size = 1
test_batch_size = 1

train_data = batchify(corpus.train, args.batch_size, args)
search_data = batchify(corpus.train, args.batch_size, args)
...
def evaluate(data_source, batch_size=1):
    ...
    hidden_RNN = np.zeros((150*450, 1, 32),dtype = float)
    flag = 0
    for i in range(0, data_source.size(0)- 1 , args.bptt):#
        data, targets = get_batch(data_source, i, args, evaluation=True)
        #data = data.view(-1)#added by lq
        #
        ...
        aa = hidden[0].data.cpu().numpy()
        aa = np.array(aa)
        
        hidden_RNN[flag,:, :] = aa
        flag = flag + 1
    #sio.savemat('hidden_RNN.mat',{'hidden':hidden_RNN})
...
def train():
    ...
    while i < train_data.size(0) - 1- 1:# 
        ...
        while start < args.batch_size:
            ...
            raw_loss = nn.functional.nll_loss(log_prob.view(-1, log_prob.size(2)), cur_targets)#log_prob is the reconstruction map
            ...
            gc.collect()
        #with open('hidden_train_search.txt', 'w') as f:
        #    for item in hidden:
        #        f.write("%s\n" % item)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(logging.info(parallel_model.genotype()))
